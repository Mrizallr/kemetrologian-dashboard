...
const TambahData: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data: pelakuUsahaData, error: pelakuError } = await supabase
        .from("pelaku_usaha")
        .insert([
          {
            ...
            status_tera: "Aktif",
            tanggal_tera_terakhir: new Date(),
          },
        ])
        .select("id");
      ...
      const pelakuUsahaId = pelakuUsahaData?.[0]?.id;

      const uttpInsert = formData.uttpList.map((uttp) => ({
        pelaku_usaha_id: pelakuUsahaId,
        jenis: uttp.jenis,
        merk: uttp.merk,
        kondisi: uttp.kondisi,
        tahun_tera: parseInt(uttp.tahun_tera),
      }));
      ...
      toast.success("Data berhasil disimpan ke Supabase!");
