...
const DaftarAlat = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const { data, error } = await supabase.from("daftar_alat").select("*");
  ...
  const columns = useMemo<ColumnDef<Alat>[]>(
    (): ColumnDef<Alat>[] => [
      ...
      },
      {
        ...
        header: "Kelas",
        cell: ({ row }): string => (row.getValue("kelas_ketelitian") as string) || "-",
      },
      {
      ...
      },
      {
        ...
        header: "Model",
        cell: ({ row }): string => (row.getValue("model_type") as string) || "-",
      },
      {
        accessorKey: "no_seri",
        header: "No Seri",
      ...
      },
      {
        ...
        cell: ({ row }): string => (row.getValue("no_sertifikat") as string) || "-",
      },
      {
        accessorKey: "tanggal_sertifikat",
        header: "Tgl Sertifikat",
        cell: ({ row }): string => (row.getValue("tanggal_sertifikat") as string) || "-",
      },
      {
        accessorKey: "rekalibrasi",
        header: "Rekalibrasi",
        cell: ({ row }): string => (row.getValue("rekalibrasi") as string) || "-",
      },
      {
        accessorKey: "lokasi_terkini",
        header: "Lokasi",
        cell: ({ row }): string => (row.getValue("lokasi_terkini") as string) || "-",
      },
      {
        id: "actions",
        header: "Aksi",
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {Object.entries(defaultForm).map(([key]) => (
                    <div key={key} className="space-y-2">
                      <Label htmlFor={key} className="text-sm font-medium">{getLabel(key)}</Label>
                      <Input
                        id={key}
                        name={key}
                        type={
                          key.includes("tanggal") || key.includes("rekalibrasi")
                            ? "date"
                            : key === "jumlah"
                            ? "number"
                            : "text"
